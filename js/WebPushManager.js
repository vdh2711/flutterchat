...
WebPushManager.prototype.start = function(callback) {
	console.log('WebPushManager start');
	if ('serviceWorker' in navigator) {		
		navigator.serviceWorker.register('js/service-worker.js').then(this.getRegistrationId(callback));
		console.log('serviceWorker in navigator');
	} else {
		callback('Service workers aren\'t supported in this browser.', null);
	}
	console.log('WebPushManager start finish runing');
}

WebPushManager.prototype.getRegistrationId = function (callback) {
	console.log('start getRegistrationId');
	navigator.serviceWorker.ready.then(
			function(serviceWorkerRegistration) {
				console.log('Run serviceWorkerRegistration');
				...
				fb_messaging.requestPermission().then(function() {
					...
					fb_messaging.getToken().then(function(currentToken) {
						if (currentToken) {
